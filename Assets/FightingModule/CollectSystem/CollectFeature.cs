...
public class CollectFeature : BaseFeature
{
    ...
    private async void OnItemCollect(ItemDetailedCollect item,Transform characterPoint, Transform leftHandPoint, Transform rightHandPoint)
    {
        ...
        await _input.MoveToPoint.Invoke(characterPoint.position);
    ...
    private void SetItemHeight(Transform leftHandPoint, Transform rightHandPoint)
    {
        ...
        {
            var localPosA = _transform.InverseTransformPoint(leftHandPoint.position);
            var localPosB = _transform.InverseTransformPoint(rightHandPoint.position);
            localY = Mathf.Lerp(localPosA.y, localPosB.y, 0.5f);
        }
        else
        {
            if(leftHandPoint)
                localY = _transform.InverseTransformPoint(leftHandPoint.position).y;
            if(rightHandPoint)
                localY = _transform.InverseTransformPoint(rightHandPoint.position).y;
        }
